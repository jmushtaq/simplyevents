...
class AbstractHirePeriod(models.Model):
    """
    Track hire periods for order lines - used for event hire system
    """

    order_line = models.OneToOneField(
        "order.Line",
        on_delete=models.CASCADE,
        related_name="hire_period",
        verbose_name=_("Order Line"),
    )
    ...
    confirmed_start = models.DateTimeField(
        ...
        null=True,
        blank=True,
        help_text=_("Staff override for actual start date"),
    )
    confirmed_end = models.DateTimeField(
        _("Confirmed End"),
        null=True,
        blank=True,
        help_text=_("Staff override for actual end date"),
    )
    ...
    class Meta:
        abstract = True
        app_label = "order"
        verbose_name = _("Hire Period")
        verbose_name_plural = _("Hire Periods")
        ordering = ["-created_at"]

    def __str__(self):
        return f"Hire: {self.start_date} to {self.end_date}"
...
class AbstractGuestHireSession(models.Model):
    """
    Track guest user hire sessions for anonymous checkout
    """
    ...
    class Meta:
        abstract = True
        app_label = "order"
        verbose_name = _("Guest Hire Session")
        verbose_name_plural = _("Guest Hire Sessions")

    def __str__(self):
        return f"Guest session: {self.email}"
