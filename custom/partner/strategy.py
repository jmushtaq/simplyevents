...
HireAvailability = get_class("partner.availability", "HireAvailability")
...
class HireSelector(Selector):
    """
    Strategy selector for hire products - uses HireAvailability
    """

    def strategy(self, request=None, user=None, **kwargs):
        return HireStrategy(request)
...
class HireStockRequired(object):
    """
    Availability policy mixin for hire products - uses HireAvailability instead of StockRequired
    """
    ...
    def availability_policy(self, product, stockrecord):
        ...
        if not product.get_product_class().track_stock:
            ...
            return HireAvailability(stockrecord.net_stock_level)
...
class FixedRateTax(object):
    ...
    stockrecord.  The price_incl_tax is quantized to four decimal places.
    Rounding behaviour is Decimal's default
    """

    rate = D("0")  # Subclass and specify the correct rate
    exponent = D("0.0001")  # Default to four decimal places
...
class HireStrategy(UseFirstStockRecord, HireStockRequired, NoTax, Structured):
    """
    Hire strategy - uses HireAvailability for Available/Unavailable messaging
    """
