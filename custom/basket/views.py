...
class BasketAddView(FormView):
    ...
    def form_valid(self, form):
        ...
        # Save hire dates from session
        hire_start = self.request.session.get('hire_start_date')
        hire_end = self.request.session.get('hire_end_date')
        if hire_start and hire_end:
            from django.utils.dateparse import parse_date
            start_date = parse_date(hire_start)
            end_date = parse_date(hire_end)
            if start_date and end_date:
                from django.utils import timezone
                self.line.hire_start_date = timezone.make_aware(
                    datetime.combine(start_date, datetime.min.time())
                )
                self.line.hire_end_date = timezone.make_aware(
